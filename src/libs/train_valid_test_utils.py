...
class TrainValidEvaluate:
    ...
    def __init__(
        ...
        self.stop_iter = train_stop_iter
        # self.writer = writer
    ...
    def sin_train(self):
        ...
        for epoch in range(1, self.n_epochs + 1):
            ...
            # TRAIN
            self.model.train()
            for i, (t, x, y) in tqdm(enumerate(self.train_loader)):
            ...
            self.model.eval()
            with torch.no_grad():
                for t, x, y in self.valid_loader:
                    output, predicted, mape, loss = self.sin_evaluate(x, y)
            ...
            print_msg = (
                f"train_loss: {train_rmse_losses:.4f} train_mape: {train_mape:.4f} || "
            ...
            if stop_train_iter > self.stop_iter:
                break
    ...
    def sin_test(self):
        self.model.eval()
        with torch.no_grad():
            for t, x, y in self.test_loader:
                output, predicted, mape, loss = self.sin_evaluate(x, y)
                self.avg_test_mape += mape
                self.avg_test_loss += loss**0.5

                for time_, input_, true_, pred_ in zip(t, x, y, predicted):
                    self.test_prediction_result.append((time_, input_, pred_))
                    self.test_target_result.append((time_, input_, true_))
        ...
        return (
            self.avg_test_loss,
            self.avg_test_mape,
            self.test_prediction_result,
            self.test_target_result,
        )
